...
public class JwtAuthenticatonConverter implements Converter<Jwt, AbstractAuthenticationToken> {
    ...
    // Fix 1: use JwtGrantedAuthoritiesConverter, not self-reference
    private final JwtGrantedAuthoritiesConverter jwtGrantedAuthoritiesConverter = new JwtGrantedAuthoritiesConverter();

    @Value("${jwt.auth.convert.principle-attribute}")
    ...
    public AbstractAuthenticationToken convert(Jwt jwt) {
        Collection<GrantedAuthority> authorities = Stream
                // Fix 2 & 3: correct converter reference, correct method name, lowercase .stream()
